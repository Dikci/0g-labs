...
[Install]
WantedBy=multi-user.target" > $HOME/0glabs.service
sudo mv $HOME/0glabs.service /etc/systemd/system
...
sudo systemctl enable 0glabs.service
sudo systemctl restart 0glabs.service


echo -e '\n\e[42mCheck node status\e[0m\n' && sleep 1
if [[ `service 0glabs.service status | grep active` =~ "running" ]]; then
  echo -e "Your 0glabs.service node \e[32minstalled and works\e[39m!"
  echo -e "You can check node status by the command \e[7mservice 0g status\e[0m"
  echo -e "Press \e[7mQ\e[0m for exit from status menu"
else
  echo -e "Your 0glabs.service node \e[31mwas not installed correctly\e[39m, please reinstall."
fi
